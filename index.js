...
module.exports = function archer(mod) {
    ...
    let AUTOWW = false;
    if (("AUTOWW" in config)) {
        AUTOWW = config.AUTOWW;
    }
    if (!("AUTOWW" in config)) {
        config.AUTOWW = false;
        config.AUTOWW_DESCRIPTION = "Auto Wind Walk after Radiant/Penetrating/TB- REQUIRES ROBOTJS, if both this and AUTOSEQFIRE are true, AUTOSEQFIRE takes priority.";
        settingUpdate();
    }

    let AUTOWW_GUST = false;
    if (("AUTOWW_GUST" in config)) {
        AUTOWW_GUST = config.AUTOWW_GUST;
    }
    if (!("AUTOWW_GUST" in config)) {
        config.AUTOWW_GUST = false;
        config.AUTOWW_GUST_DESCRIPTION = "Auto Wind Walk after Gust arrow - REQUIRES ROBOTJS, if both this and AUTOSEQFIRE_GUST are true, AUTOSEQFIRE_GUST takes priority.";
        settingUpdate();
    }
    ...
    let DISABLE_GUST = false;
    if (("DISABLE_GUST" in config)) {
        DISABLE_GUST = config.DISABLE_GUST;
    }
    if (!("DISABLE_GUST" in config)) {
        config.DISABLE_GUST = false;
        config.DISABLE_GUST_DESCRIPTION = "Disable gust arrow emulation.";
        settingUpdate();
    }
    ...
    let FD_CANCEL = false;
    if (("FD_CANCEL" in config)) {
        FD_CANCEL = config.FD_CANCEL;
    }
    if (!("FD_CANCEL" in config)) {
        config.FD_CANCEL = false;
        config.FD_CANCEL_DESCRIPTION = "Automatically move cancel Feign Death as soon as you're out of combat. Toggle state in game with fd1 command.";
        settingUpdate();
    }
    ...
    mod.hook('C_CHAT', 1, (event) => {
        if (event.message.includes("disable5")) {
            enabled = false;
            mod.command.message("Archer script disabled");
            return false;
        }
        if (event.message.includes("enable5")) {
            enabled = [JOB_ARCHER].includes(job);
            mod.command.message("Archer script enabled if you are currently logged into archer");
            return false;
        }
        if(event.message.includes("fd1")){
            FD_CANCEL = !FD_CANCEL;
            mod.command.message("FD_CANCEL state: "+FD_CANCEL);
            return false;
        }
    });
    ...
    function fakeDB(event, duration) {
        ...
        if (event.skill.id == GALE1) {
            mod.toClient('S_ACTION_STAGE', 9, {
                ...
                id: atkid[event.skill.id],
                effectScale: 1.0, moving: false, dest: { x: 0, y: 0, Z: 0 }, target: 0n,
                animSeq: [{ duration: 766, xyRate: 2, zRate: 1, distance: 0.01 }],
            });
        }
    ...
    mod.hook('S_EACH_SKILL_RESULT', mod.majorPatchVersion >= 110 ? 15 : 14, (event) => {
        if (event.target === cid) {
    ...
    function fakeEnd_sorc_dist(event, duration, dist) {
        ...
        var yyy =1;
        var zzz =0;
        if ((event.skill.id == S_ARROWRAIN || event.skill.id == (S_ARROWRAIN +50)) && glyphState[26077] ==1) {
            yyy = 1.4;
        }
        ...
        if ((event.skill.id == S_ARROWRAIN || event.skill.id == (S_ARROWRAIN +50)) && (noct1 || noct2 || noct3 || noct4)) {
            yyy = yyy * 1.225;
        }
        if ((event.skill.id == S_ARROWRAIN || event.skill.id == (S_ARROWRAIN +50)) && (noct5 || noct6 || noct7 || noct8 || noct9 || noct10 || noct11 || noct12 || noct13)) {
            yyy = yyy * 1.15;
        }
        if (finish[lastSkill] == false && lastSkill != event.skill.id && zzz ==0) {
            force_end(lastEvent,4);
            finish[lastSkill] = true;
        }
        if (event.skill.id == S_TENA) {
            yyy = 1 / aspd;
        }
        ...
        if ((event.skill.id == S_ARROWRAIN || event.skill.id == (S_ARROWRAIN +50)) && talentState[860540]) {
        ...
        timer[event.skill.id] = setTimeout(
            function (event) {
                ...
                if ((event.skill.id == S_BACKSTEP || event.skill.id == S_BREAKAWAY) && (lastSkill == S_BACKSTEP || lastSkill == S_BREAKAWAY)) {
    ...
    mod.hook('S_CAN_LOCKON_TARGET', 3, { order: 99999, filter: { fake: true } }, (event) => {
        if (!enabled) { return };
        if (AUTOVOLLEY && lastSkill == S_ARROWVOLLEY) {
            flameN++;
    ...
    mod.hook('S_CREST_MESSAGE', 2, { order: -99999 }, (event) => {
        if (!enabled) { return };
        if (RF_RESET_STOP && lastSkill != Breeze_Steriod && event.skill.id == (S_RAPIDFIRE)) {
            mod.toClient('S_START_COOLTIME_SKILL', 3, {
                skill: S_RAPIDFIRE,
                cooldown: (20 + 2500 / aspd),
            });
        }
    ...
    mod.hook('C_START_TARGETED_SKILL', 7, (event) => {
        ...
        if (!disabSkill[event.skill.id]) {
            ...
            setTimeout(function () { mod.toServer('C_START_TARGETED_SKILL', 7, event); },25);
            setTimeout(function () { mod.toServer('C_START_TARGETED_SKILL', 7, event); },50);
            setTimeout(function () { mod.toServer('C_START_TARGETED_SKILL', 7, event); },75);
            setTimeout(function () { mod.toServer('C_START_TARGETED_SKILL', 7, event); },100);
    ...
    function grantskill(xskill) {
        ...
        mod.toClient('S_GRANT_SKILL', 3, {
            skill: xskill,
        });
        lastspoof = xskill;
    }

    mod.hook('S_GRANT_SKILL', 3, (event) => {
        if (!enabled) { return };
    ...
    mod.hook('C_PRESS_SKILL', 4, (event) => {
        ...
        if (job == JOB_ARCHER && event.press == true) {
            ...
            setTimeout(function (event) { if ((lastSkill == TEMPEST_TRASH_ARROW && TrashActive ==0) || lastSkill != event.skill.id) { return; } mod.toServer('C_PRESS_SKILL', 4, event); }, 50, event);
            setTimeout(function (event) { if ((lastSkill == TEMPEST_TRASH_ARROW && TrashActive ==0) || lastSkill != event.skill.id) { return; } mod.toServer('C_PRESS_SKILL', 4, event); }, 100, event);
            setTimeout(function (event) { if ((lastSkill == TEMPEST_TRASH_ARROW && TrashActive ==0) || lastSkill != event.skill.id) { return; } mod.toServer('C_PRESS_SKILL', 4, event); }, 150, event);
            setTimeout(function (event) { if ((lastSkill == TEMPEST_TRASH_ARROW && TrashActive ==0) || lastSkill != event.skill.id) { return; } mod.toServer('C_PRESS_SKILL', 4, event); }, 200, event);
            chargeSkillFix[chargeSkillIDx] = setInterval(function (event, nnn) {
                if ((event.skill.id == S_PENARROW || event.skill.id == S_RADIANT) && lastSkill == event.skill.id && event.press == true && chargeSkillFix2 != lastSkill) {
                    mod.toServer('C_PRESS_SKILL', 4, event);
                }
        ...
        if (job == JOB_ARCHER && (event.skill.id == S_RADIANT || event.skill.id == S_PENARROW || event.skill.id == TEMPEST_TRASH_ARROW) && event.press == false) {
            ...
            if (stallSorc >10) {
                mod.command.message('attempt unstuck');
                mod.toClient('S_ACTION_END', 5, {
                    gameId: cid,
                    loc: { x: event.loc.x, y: event.loc.y, z: event.loc.z },
                    w: event.w,
        ...
        if (job == JOB_ARCHER && event.skill.id == TEMPEST_TRASH_ARROW && TrashActive ==1) {
            if (DISABLE_CHARGE || DISABLE_GUST) { lastSkill = TEMPEST_TRASH_ARROW; return; }
            fakeEnd_AP(event, 0, event.loc.x, event.loc.y, event.loc.z);
            stallSorc =0;
            APStateZZ =1;
    ...
    mod.hook('S_CANNOT_START_SKILL', 4, (event) => {
        if (!enabled) { return };
        if(event.skill.id == S_ARROWVOLLEY){
            mod.toServer('C_START_SKILL', 7, lastvolley);
        }
    });
    ...
    mod.hook('S_ABNORMALITY_BEGIN', 4, (event) => {
        ...
        if (event.id ==602108) {
            melody = true;
            setTimeout(function () { melody = false; }, Number(event.duration));
        }
        if (event.id ==602221) {
            melody2 = true;
            setTimeout(function () { melody2 = false; }, Number(event.duration));
        }
        if(event.id ==920){
            noct1 = true;
        }
    ...
    mod.hook('S_USER_STATUS', 4, (event) => {
        if(!enabled) return;
        if(sDeath && event.status == 0 && event.gameId === cid && FD_CANCEL){
        var robot345 = require("robotjs");
             robot345.keyTap("s");
        }
    });
    ...
    mod.hook('C_START_COMBO_INSTANT_SKILL', 6, (event) => {
        ...
        if (!disabSkill[event.skill.id]) {
            lastSkillDelay =999999;
            setTimeout(function (event) { mod.toServer('C_START_COMBO_INSTANT_SKILL', 6, event); }, 25, event);
            setTimeout(function (event) { mod.toServer('C_START_COMBO_INSTANT_SKILL', 6, event); }, 50, event);
            setTimeout(function (event) { mod.toServer('C_START_COMBO_INSTANT_SKILL', 6, event); }, 75, event);
            setTimeout(function (event) { mod.toServer('C_START_COMBO_INSTANT_SKILL', 6, event); }, 100, event);
            setTimeout(function (event) { mod.toServer('C_START_COMBO_INSTANT_SKILL', 6, event); }, 125, event);
            setTimeout(function (event) { mod.toServer('C_START_COMBO_INSTANT_SKILL', 6, event); }, 150, event);
            ...
            if (job == JOB_ARCHER && event.skill.id == S_RAPIDFIRE_B) {
                disabSkill[event.skill.id] = true;
                var timer = setTimeout(function () { disabSkill[S_RAPIDFIRE_B] = false; }, GLOBAL_LOCK_DELAY );
                fakeDB(event, S_RAPIDFIRE_B_D);
            }

            if (job == JOB_ARCHER && event.skill.id == S_RAPIDFIRE_B2) {
                disabSkill[event.skill.id] = true;
                var timer = setTimeout(function () { disabSkill[S_RAPIDFIRE_B2] = false; }, GLOBAL_LOCK_DELAY );
                fakeDB(event, S_RAPIDFIRE_B2_D);
            }

            if (job == JOB_ARCHER && event.skill.id == S_RAPIDFIRE_B3) {
                disabSkill[event.skill.id] = true;
                var timer = setTimeout(function () { disabSkill[S_RAPIDFIRE_B3] = false; }, GLOBAL_LOCK_DELAY );
                fakeDB(event, S_RAPIDFIRE_B3_D);
            }

            if (job == JOB_ARCHER && event.skill.id == S_RAPIDFIRE_B4) {
                disabSkill[event.skill.id] = true;
                var timer = setTimeout(function () { disabSkill[S_RAPIDFIRE_B4] = false; }, GLOBAL_LOCK_DELAY );
    ...
    mod.hook('C_START_INSTANCE_SKILL', 7, (event) => {
        ...
        if (!disabSkill[event.skill.id]) {
            ...
            if (!(event.skill.id == GALE1 || event.skill.id == GALE2 || event.skill.id == GALE3 || event.skill.id == GALE4 || event.skill.id == GALE5 || event.skill.id == GALE6 || event.skill.id == GALE7 || event.skill.id == GALE8 || event.skill.id == GALE9)) {
                setTimeout(function (event) { if (lastSkill == event.skill.id) { mod.toServer('C_START_INSTANCE_SKILL', 7, event); } }, 25, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { mod.toServer('C_START_INSTANCE_SKILL', 7, event); } }, 50, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { mod.toServer('C_START_INSTANCE_SKILL', 7, event); } }, 75, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { mod.toServer('C_START_INSTANCE_SKILL', 7, event); } }, 100, event);
            }
            ...
            if (event.skill.id == S_PENARROW_1 || event.skill.id == S_PENARROW_2 || event.skill.id == S_PENARROW_3 || event.skill.id == S_PENARROW_4) {
                ...
                if (AUTOWW && (seqfiredisable || !AUTOSEQFIRE)) {
                    lastSkill = event.skill.id;
                    failsafe =0;
                    setTimeout(function(event){
                    repeater(WWKEY, event.skill.id);
                    }, 100 / aspd, event);
                }
            }
    ...
    mod.hook('C_START_SKILL', 7, (event) => {
        ...
        if(event.skill.id == (S_ARROWRAIN +50) && locking){
            return false;
        }
        ...
        if (!disabSkill[event.skill.id] && (finish[SKILL_CHARGING] != false || event.skill.id == S_BACKSTEP)) {
            ...
            if (xzzy && event.skill.id != S_ARROWVOLLEY && event.skill.id != S_RETAL) {
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} mod.toServer('C_START_SKILL', 7, event); } }, 25, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} mod.toServer('C_START_SKILL', 7, event); } }, 50, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} mod.toServer('C_START_SKILL', 7, event); } }, 75, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} mod.toServer('C_START_SKILL', 7, event); } }, 100, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} mod.toServer('C_START_SKILL', 7, event); } }, 150, event);
                setTimeout(function (event) { if (lastSkill == event.skill.id) { if(lastarrow == 50 && event.skill.id == S_ARROWRAIN){event.skill.id = (S_ARROWRAIN +50)} mod.toServer('C_START_SKILL', 7, event); } }, 200, event);
            }
            lastarrow =0;
            if(event.skill.id == (S_ARROWRAIN +50)){
                //event.skill.id = S_ARROWRAIN;
                lastarrow =50;
            }
            ...
            if (job == JOB_ARCHER && event.skill.id == Breeze_Steriod) {
                ...
                mod.toClient('S_ABNORMALITY_BEGIN', 4, {
                    ...
                    stacks: 1,
                    unk2: 0,
                    ...(mod.majorPatchVersion >= 110 ? { unk3: 0 } : 0n),
                });
                disabSkill[event.skill.id] = true;
            ...
            if (job == JOB_ARCHER && event.skill.id == (TEMPEST_TRASH_ARROW_R -1)) {
                ...
                if (AUTOWW_GUST && (seqfiredisable || !AUTOSEQFIRE_GUST)) {
                    lastSkill = event.skill.id;
                    failsafe =1;
                    setTimeout(function(event){
                    repeater(WWKEY, event.skill.id);
            ...
            if (job == JOB_ARCHER && event.skill.id == S_THUNDER) {
                clearTimeout(thunderdis);
                disabSkill[S_THUNDER] = true;
                thunderdis = setTimeout(function () { disabSkill[S_THUNDER] = false; }, GLOBAL_LOCK_DELAY *2);
            ...
            if (job == JOB_ARCHER && event.skill.id == S_ARROWRAIN) {
                ...
                disabSkill[(S_ARROWRAIN +50)] = false;
            ...
            if (job == JOB_ARCHER && event.skill.id == (S_ARROWRAIN +50)) {
                disabSkill[S_ARROWRAIN] = false;
                disabSkill[(S_ARROWRAIN +50)] = false;
                clearTimeout(raining);
                disabSkill[event.skill.id] = true;
                raining = setTimeout(function () { disabSkill[(S_ARROWRAIN +50)] = false; }, GLOBAL_LOCK_DELAY *10);
                fakeEnd_sorc_dist(event, S_ARROWRAIN_D,0);
            }
    ...
    mod.hook('C_CANCEL_SKILL', 3, (event) => {
        ...
        if (job == JOB_ARCHER && (event.skill.id == S_ARROWRAIN || event.skill.id == (S_ARROWRAIN +50)) && !ArrowRainDisab) {
            clearTimeout(raining);
            disabSkill[S_ARROWRAIN] = false;
disabSkill[(S_ARROWRAIN +50)] = false;
            clearTimeout(timer[lastSkill]);
        ...
        if(lastarrow == 50 && lastSkill == S_ARROWRAIN){
            event.skill.id = (S_ARROWRAIN +50);
            return true;
        }
    });

    mod.hook('S_ACTION_STAGE', 9, (event) => {
      //mod.command.message("test: " + event.skill == 67114610);
        if (!enabled) return;
        if (event.gameId === cid) {
            ...
            if (job == JOB_ARCHER && (event.skill.id == S_PENARROW || event.skill.id == S_RADIANT || event.skill.id == TEMPEST_TRASH_ARROW)) {
                ...
                if(DISABLE_GUST && event.skill.id == TEMPEST_TRASH_ARROW){return;}
                return false;
            }
            if (job == JOB_ARCHER && (event.skill.id == S_ARROWRAIN || event.skill.id == (S_ARROWRAIN +50))) {
                return false;
            }
    ...
    mod.hook('S_ACTION_END', 5, (event) => {
      if(event.skill == 67108526) mod.command.message("error");
        if (!enabled) return;
        if (event.gameId === cid) {
            var d = new Date();
            ...
            if (((lastSkillTime[2] - lastSkillTime[1]) > lastLastSkillDelay) && (event.skill.id == lastLastSkill || event.skill.id == lastLastSkill +30) && (lastLastSkill == lastSkillTime[3] || lastLastSkill == (lastSkillTime[3] -30))) {
                ...
                RecheckTimer = setTimeout(function () {
                    ...
                    mod.toClient('S_ACTION_END', 5, lastSkillEvent);
            ...
            if (job == JOB_ARCHER && (event.skill.id == TEMPEST_TRASH_ARROW)) {
                TrashActive =0;
                if (DISABLE_CHARGE || DISABLE_GUST) { return; }
                return false;
            }
            if (job == JOB_ARCHER && (event.skill.id == S_ARROWRAIN || event.skill.id == (S_ARROWRAIN +50))) {
                return false;
    ...
    mod.hook('S_START_COOLTIME_SKILL', 3, (event) => {
        ...
        if(event.skill.id == S_SEQFIRE){
            clearTimeout(locking99);
            seqfiredisable = true;
            locking99 = setTimeout(function(){seqfiredisable = false;}, event.cooldown);
        }
        return true;
    });
